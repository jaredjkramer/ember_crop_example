...
App.IndexController = Ember.ObjectController.extend(Ember.Evented, {
    ...
    createCropName: function (img) {
        var path = img.imagepath,
            curTime = this.renderTime(),
            ext = path.substr(path.lastIndexOf(".")),
            filename = path.substr(path.lastIndexOf("/") + 1);
        path = path.replace(filename, "");
        return path + filename.replace(ext, "") +  "_CROP" + ext; //"_" + curTime +
    },
    ...
    actions: {
        commitImage: function (img) {
          if (!this.cropped) {
            return siteNoty("<strong>Error:</strong> You did not crop the image", "error", "center");
          }
            if (this.currentMainImage != null) {
                var upload = this.store.createRecord('upload', {}),
                    uploaddata = $('#editedImage').attr('src'),
                    newname = this.createHeaderName(img),
                    controller = this;
        ...
        chooseImage: function (image) {
            ...
            if (this.initCrop === false && this.currentMainImage != {}) {
                var params = {
                    ratio: (this.mainImageSize.width / this.mainImageSize.height),
                    minH: this.mainImageSize.height,
                    minW: this.mainImageSize.width,
                    resize: false
                };
...
App.IndexView = Ember.View.extend({
    ...
    actions: {
        ...
        commitCrop: function (image) {
            ...
            this.controller.confirmCrop();
            $('#imageCropPanel').trigger('click');
            return siteNoty("<strong>Success!:</strong> Here is your cropped image.<br>Select Commit Image from the Toolbar to Confirm and Exit.", "success", "center")
        }
    }
